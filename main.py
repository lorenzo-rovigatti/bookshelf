...
from zhengli.core.detect_books_from_shelf import (find_rectangles, get_book_lines, make_crops_from_rect)
...
parser.add_argument('-f', action='store', dest='file_path', help='shelf image file path')

inputs = parser.parse_args()

# python zhengli/core/main.py -f IMG_20190904_155939359.jpg

PAYLOAD_DIR = join(abspath(dirname(dirname(dirname((__file__))))), "zhengli/data")
#IMAGE_FILE = join(PAYLOAD_DIR, inputs.file_path)
IMAGE_FILE = join(PAYLOAD_DIR, "prova.jpg")
...
def main():
    ...
    books_info = []
    for _, book in storage_book.items():
        title = max(book[:-1], key=len)
        #print('title is {}'.format(title))
        ddc, dict_of_results = get_ddc_api(title=title, detected_text=book, pub_list_path=PUBLISHER_LIST)
